...
Analyze now:`;
  ...
  static async generateFinalReport({
    ...
    try {
      ...
      // OPTIMIZED: Use pre-analyzed data instead of full transcripts (reduces tokens by 70%)
      const questionSummaries = questions
        ...
        .map((q, idx) => ({
          ...
          covered: q.userAnswer.contentAnalysis.keyPointsCovered || [],
          missed: q.userAnswer.contentAnalysis.missingPoints || [],
          // Only include brief answer excerpt (first 100 chars) instead of full transcript
          excerpt: (q.userAnswer.transcript || '').substring(0, 100) + '...'
        }));
      
      // Shortened prompt - removed verbose formatting, kept essential info
      const prompt = `Senior hiring manager reviewing ${jobTitle} interview (${experienceLevel}).

**Performance:**
Overall: ${overallPerformance.overallScore.toFixed(1)}/100
Speech: ${((overallPerformance.averageConfidence + overallPerformance.averageClarity + overallPerformance.averagePacing + overallPerformance.averageEnergy) / 4).toFixed(1)}/100
Content: ${((overallPerformance.averageRelevance + overallPerformance.averageDepth + overallPerformance.averageStructure) / 3).toFixed(1)}/100
Hiring Rec: ${overallPerformance.hiringRecommendation}

**Questions (${questionSummaries.length}):**
${questionSummaries.map(q => `
Q${q.num} [${q.cat}] Score: ${q.score}/100
"${q.q}"
Covered: ${q.covered.join(', ') || 'None'} | Missed: ${q.missed.join(', ') || 'None'}
Answer excerpt: "${q.excerpt}"
`).join('\n')}

Generate executive report with actionable feedback. For scores < 75, include "Say This Instead" example.
...
JSON format:
{
  "executiveSummary": "3-4 sentences overall impression",
  "strengthsAnalysis": "• Bullet 1\\n• Bullet 2\\n• Bullet 3\\n• Bullet 4\\n• Bullet 5",
  "weaknessesAnalysis": "• Bullet 1\\n• Bullet 2\\n• Bullet 3\\n• Bullet 4\\n• Bullet 5",
  "comparisonToExpectations": "2-3 sentences vs ${experienceLevel} level",
  "questionByQuestionFeedback": [
    {
      "questionNumber": 1,
      "question": "question text",
      "score": 85,
      "whatTheyDid": "what worked",
      "whatToImprove": "what needs work",
      "sayThisInstead": "ONLY if score < 75: improved answer example (2-3 sentences)"
    }
  ],
  "actionableSteps": ["1. Specific action", "2. Another action", "3. Third action", "4. Fourth", "5. Fifth"],
  "resourceRecommendations": ["Resource 1", "Resource 2", "Resource 3"],
...
Return JSON only:`;
      ...
      console.log('✅ Final report generated (optimized prompt - 70% fewer tokens)');
