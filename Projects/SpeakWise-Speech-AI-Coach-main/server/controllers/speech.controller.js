...
const { transcribeWithWhisperLocal, isWhisperLocalAvailable } = require('../services/whisperLocal.service');

// ASR Provider Configuration
// Set ASR_PROVIDER environment variable to 'whisper', 'whisper-local', or 'google' (default: 'google')
const ASR_PROVIDER = (process.env.ASR_PROVIDER || 'google').toLowerCase();
...
if (ASR_PROVIDER === 'google') {
  try {
    ...
    console.log('✅ Using Google Speech-to-Text as ASR provider');
  } catch (error) {
    console.error('❌ Failed to initialize Google Speech client:', error.message);
  }
} else if (ASR_PROVIDER === 'whisper') {
  console.log('✅ Using OpenAI Whisper API as ASR provider');
  if (!isWhisperAvailable()) {
    console.error('❌ Whisper API not available. Please set OPENAI_API_KEY environment variable.');
  }
} else if (ASR_PROVIDER === 'whisper-local') {
  console.log('✅ Using Local Whisper (open-source) as ASR provider');
...
exports.analyzeSpeech = async (req, res) => {
  ...
  try {
    ...
    // 1. Transcribe audio using selected ASR provider
    ...
    if (ASR_PROVIDER === 'whisper') {
      ...
      if (!isWhisperAvailable()) {
        return res.status(500).json({
          success: false,
          message: 'Whisper API is not available. Please configure OPENAI_API_KEY.'
        });
      }
      ...
      // Use Local Whisper (open-source)
      console.log('🎤 Using Local Whisper (open-source) for transcription...');
      
      const isAvailable = await isWhisperLocalAvailable();
      if (!isAvailable) {
        return res.status(500).json({
          success: false,
          message: 'Local Whisper is not installed. Please run: pip install -U openai-whisper'
        });
      }

      const whisperResult = await transcribeWithWhisperLocal(req.file.buffer, req.file.originalname);
      transcript = whisperResult.transcript;
      words = whisperResult.words;
